...
// the poly AFTERTOUCH lane (from Core MIDI-CV / AlSplitter), one output per
...
struct AlAftertouch : Module {
    ...
    AlAftertouch() {
        ...
        for (int id = 0; id < 16; id++)
            configOutput(AFT_OUTPUTS + id, string::f("Aftertouch %d", id + 1));
    ...
    void process(const ProcessArgs&) override {
        ...
        for (int id = 0; id < 16; id++) {
            ...
            outputs[AFT_OUTPUTS + id].setChannels(1);
            outputs[AFT_OUTPUTS + id].setVoltage(c >= 0 ? inputs[AFT_INPUT].getVoltage(c) : 0.f);
        }
...
struct AlAftertouchWidget : ModuleWidget {
    AlAftertouchWidget(AlAftertouch* module) {
        ...
        setPanel(new AlPanel(mm2px(Vec(panelWidth, 128.5f)),
            Svg::load(asset::plugin(pluginInstance, "res/AlAftertouch_Silk_Light.svg")),
            Svg::load(asset::plugin(pluginInstance, "res/AlAftertouch_Silk_Dark.svg"))));
